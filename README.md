...
All generated by `tools/build-hero.py` from the client's two cover photographs. The masters live in `~/Desktop/Clients 🔒/Unscripted Travel/` and are not committed; pass a path as the first argument if they move.

- `booklet-wide.jpg` — 2600×1122, desktop. Paper extended left/right so the sheet bleeds full width.
- `booklet-tight.jpg` — 1199×968, mobile (≤700px). Cropped close to the book; the pinned stage supplies the white around it.
...
The booklet was shot on kraft. The build script fits a quadratic lighting field to the clean paper only — the book and the area its shadow falls into are excluded — then divides every background pixel by that field. Clean paper lands on white; the shadow keeps its falloff and becomes a soft neutral grey. The book itself is untouched.

Two follow-on details matter:

- Dividing by a darker part of the field amplifies its noise, which shows as blotching. A smooth highlight ramp pulls near-white values the rest of the way to white, so the paper lands flat (std under 0.1 levels) while the shadow keeps its gradient.
- Because the sheet is then uniform, the margins are extended with a **flat fill**, not mirror-tiled. Tiling grain that no longer exists only reintroduced banding.
