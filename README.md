...
The reveal is a `mask-image` gradient whose position is a single registered custom property (`@property --wipe`), written once per frame from a rAF. Measured over a fast sweep: 325 frames, median 16.70ms, worst 16.80ms, zero drops.
